...
struct MenuPage: View{
    ...
    var body: some View{
        ZStack{
            ...
            if (basket.showPopup == "AddedToBasket") {
                popupWindow(image: "GreenTick", text: "Added to Basket!")
                    .onAppear{
...
class AllItems: ObservableObject {
    ...
    func fetchAllItems() {
        ...
        db.collection("Items").addSnapshotListener { querySnapshot, error in //listens for changes in 'Items'
            ...
            self.all = documents.map { Item(id: $0.documentID, name: "\($0["Name"]!)" , desc: "\($0["Description"]!)", cost: "\($0["Sales Price"]!)", category: "\($0["Category"]!)", image: "\($0["Image"]!)", stock: "\($0["Stock"]!)", count: 0) // adds in all Items as type 'Item' and their details into the 'all' list
                // debug: $0 is first parameter
                
            }
        print("Items: \(self.all)") // debug - return all the items into the console
